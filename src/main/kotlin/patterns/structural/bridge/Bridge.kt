package team.idivision.samples.designpatterns.patterns.structural.bridge
...
import team.idivision.samples.designpatterns.patterns.structural.bridge.message.LongMessage
import team.idivision.samples.designpatterns.patterns.structural.bridge.message.ShortMessage
import team.idivision.samples.designpatterns.patterns.structural.bridge.sender.EmailMessageSender
import team.idivision.samples.designpatterns.patterns.structural.bridge.sender.SenderType
import team.idivision.samples.designpatterns.patterns.structural.bridge.sender.SmsMessageSender
...
class Bridge(
    ...
    override fun launch() {
        val sender: MessageSender = when (senderType) {
            SenderType.EMAIL -> EmailMessageSender()
            SenderType.SMS -> SmsMessageSender()
        }

        val message: AbstractMessage = when (senderType) {
            SenderType.EMAIL -> LongMessage(sender)
            SenderType.SMS -> ShortMessage(sender)
        }

        message.sendMessage("Bridge established. Works fine.")
    }
}