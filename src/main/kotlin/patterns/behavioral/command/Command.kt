package team.idivision.samples.designpatterns.patterns.behavioral.command

import team.idivision.samples.designpatterns.patterns.Pattern
import team.idivision.samples.designpatterns.patterns.behavioral.command.cmd.AddCommand
import team.idivision.samples.designpatterns.patterns.behavioral.command.cmd.SubCommand
import team.idivision.samples.designpatterns.patterns.behavioral.command.invoker.Result
import team.idivision.samples.designpatterns.patterns.behavioral.command.util.CommandHistory
...
class Command : Pattern("Command") {
    ...
    override fun launch() {
        ...
        if (subCommand.execute(20)) {
            commandHistory.push(subCommand)
            result.preview()
        }

        if (undo(commandHistory)) {
            result.preview()
        }
    }

    private fun undo(history: CommandHistory): Boolean {
        if (history.isEmpty()) {
            return false
        }
